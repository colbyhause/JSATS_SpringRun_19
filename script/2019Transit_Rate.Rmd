...
change_receiver_names <- function(df) {
  ...
  df$'GPS Names'[df$'GPS Names' == "SJ_Moss_2_J"] <- "Moss"
  df$'GPS Names'[df$'GPS Names' == "SJ_Moss_1_J"] <- "Moss"
...
5. Run function below to get calculate the transit rates. Determine transit rate by calculating the rate of each detection line, but then selecting only the detections we want ( last detection at the first receiver and first detection at the second, then last detection at the second and first detection at the third, etc...) via an if statement.
...
order_route<- function(route_vector, transit_rate_df, locations_df, pdf_name) { 
  ...
  for (route in route_vector) {
    ...
    if (loc_colname ==  "Upstream Release Mainstem Route To Ocean ") {
      medians$transit_rate_final[6] <- 55
