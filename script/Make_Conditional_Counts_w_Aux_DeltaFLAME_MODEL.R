...
# THIS CODE IS TO GET COUNTS FOR THE NEW FLAME VERSION OF THE DELTA MODEL, which is combined with the Full model. : IE. the model designed to just focus on survival estimates from the 
...
data <- read_csv("data_output/DeltaRel_visits2019_firstFLAME_MODEL_w_Chipps_092419.csv") # no B1 ->JP detection histories

# Make a vector of locations that are included in model 

locs <- c('REL', 'A7', 'A8', 'A9', 'A10', 'A11', 'B1',
...
write.csv(input, 'data_output/User_Model_Input/2019_DeltaRelease_counts_FLAME_MODEL_w_Chipps_092419.csv')
...
write.csv(Aux, "data_output/User_Model_Input/2019_DeltaRel_Cond_Aux_counts_FLAME_MODEL_w_Chipps_092419.csv")
